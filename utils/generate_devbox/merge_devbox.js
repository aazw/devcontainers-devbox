...
// 各ファイルからパッケージ名のみを抽出してマージ
const allPackages = devboxConfigs.flatMap(config => config.parsed.packages || []);

// パッケージ名をインデント付きでフォーマット
const packagesFormatted = allPackages.map((pkg, index) => {
  const comma = index < allPackages.length - 1 ? ',' : '';
  return `    "${pkg}"${comma}`;
});

// 生成元ファイルのリストを作成
const sourceFiles = inputFiles.join(' and ');

// 出力を構築（先頭に生成元のコメントを追加）
const output = `// generated by merge ${sourceFiles}
{
  "$schema": "${schema}",
  "packages": [
${packagesFormatted.join('\n')}
  ]
}
`;
